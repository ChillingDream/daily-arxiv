    ...
    with gr.Tab("关键词设置"):
        with gr.Row():
            with gr.Column():
                ...
                word_to_delete_input = gr.Dropdown(
                    label="选择要删除的词语", choices=words_list
                )
                delete_button = gr.Button("删除词语")

            words_display = gr.Textbox(
                label="当前词语列表", lines=10, interactive=False
            )
        ...
        # 删除词语按钮的事件处理
        delete_button.click(
            delete_word, inputs=word_to_delete_input, outputs=words_display
        ).then(lambda: gr.Dropdown(choices=words_list), outputs=word_to_delete_input)
