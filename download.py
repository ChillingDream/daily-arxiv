...
def update(theday, start_id, max_result):
    ...
    id_list = ','.join(['{:02}{:02}.{:05}'.format(theday.year % 100, theday.month, number) for number in range(start_id_number + 1, start_id_number + max_result + 1)])
    url = 'http://export.arxiv.org/api/query?id_list={}&max_results={}'.format(id_list, max_result)
    ...
    with libreq.urlopen(url) as connection:
        ...
        for child in root:
            if child.tag == 'entry':
                ...
                raw_articles.append({"arxiv_id": id, "text": ET.tostring(child).decode('utf-8')})

                title = child.find('title').text
                submitted_date = child.find('published').text
                submitted_date = datetime.datetime.strptime(submitted_date, '%Y-%m-%dT%H:%M:%SZ')
                ...
                if category.startswith('cs'):
                    articles.append({
                        "arxiv_id": id,
                        "added_date": datetime.datetime(theday.year, theday.month, theday.day, 0, 0, 0),
